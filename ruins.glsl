...
vec4 noise4(in vec2 v) { return texture2D(us2_noise, v / 256., -100.); }
float hash(float v) { return texture2D(us2_noise, vec2(v) / 256., -100.).r; }
float hash(in vec2 v) { return texture2D(us2_noise, (v + .5) / 256., -100.).r; }
float noise(in vec2 v) { return texture2D(us2_noise, v / 256., -100.).r; }
float noise(in vec3 v) {
	v /= 256.;
	return .5 * (
		texture2D(us2_noise, v.xz, -100.).b +
		texture2D(us2_noise, v.yx, -100.).g);
}
...
vec3 noise3(in vec2 v) { return texture2D(us2_noise, v / 256., -100.).rgb; }
vec3 noise3(in vec3 v) {
	v /= 256.;
	return .5 * (
		texture2D(us2_noise, v.xz, -100.).rgb +
		texture2D(us2_noise, v.yx, -100.).abg);
}
...
float H(in vec3 at) {
	vec2 a = at.xz / 256. * .02;
	return 9.-24. * (
		.5 * texture2D(us2_noise, a, -100.).r +
		.25 * texture2D(us2_noise, a * 1.93, -100.).r +
		.0625 * texture2D(us2_noise, a * 8.07, -100.).r);
}
...
void main() {
	...
	vec2 uv = 2. * gl_FragCoord.xy / uv2_resolution - vec2(1.);
